...
class TestBottleNetCDF(TestCase):
    ...
    def test_write(self):
        ...
        self.file['EXPOCODE'] = libcchdo.model.datafile.Column('EXPOCODE')
        self.file['EXPOCODE'].append('TESTEXPO')

        self.file['SECT_ID'] = libcchdo.model.datafile.Column('SECT_ID')
        self.file['SECT_ID'].append('TEST')

        self.file['STNNBR'] = libcchdo.model.datafile.Column('CASTNO')
        self.file['STNNBR'].append(5)

        self.file['CASTNO'] = libcchdo.model.datafile.Column('STNNBR')
        self.file['CASTNO'].append(20)

        self.file['DEPTH'] = libcchdo.model.datafile.Column('DEPTH')
        self.file['DEPTH'].append(-1)

        self.file['LATITUDE'] = libcchdo.model.datafile.Column('LATITUDE')
        self.file['LATITUDE'].append(90)

        self.file['LONGITUDE'] = libcchdo.model.datafile.Column('LONGITUDE')
        self.file['LONGITUDE'].append(180)

        self.file['_DATETIME'] = libcchdo.model.datafile.Column('_DATETIME')
        self.file['_DATETIME'].append(datetime.datetime.utcnow())

        self.file['BTLNBR'] = libcchdo.model.datafile.Column('BTLNBR')
        self.file['BTLNBR'].append(5, 9)

        self.file['CTDOXY'] = libcchdo.model.datafile.Column('CTDOXY')
        ...
        netcdfbot.write(self.file, tempfile.NamedTemporaryFile())
