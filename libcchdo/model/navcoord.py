...
def iter_coords(self, navcoordcls, callback):
    ...
    if (isinstance(self, DataFile) or isinstance(self, SummaryFile)):
        ...
        for expocode in expocodes:
            ...
            lons = [self['LONGITUDE'][i] for i in indices]
            lats = [self['LATITUDE'][i] for i in indices]
            stnnbrs = [self['STNNBR'][i] for i in indices]
            dates = [self['_DATETIME'][i] for i in indices]
            try:
                codes = [self['_CODE'][i] for i in indices]
            except KeyError:
                codes = [None] * len(lons)
            items = zip(lons, lats, stnnbrs, dates, codes)
            try:
                expocode_coords[expocode].append(items)
            except KeyError:
