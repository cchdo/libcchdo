...
try:
  ...
  raise ImportError('%s\n%s' % (e, '''You should get netcdf4-python from
  http://code.google.com/p/netcdf4-python and install the NetCDF 3 module as
  directed by the README.'''))
try:
  import pgdb
except ImportError, e:
  raise ImportError('%s\n%s' % (e, '''You should get pygresql from
    http://www.pygresql.org/readme.html#where-to-get. You will need
    Postgresql with server binaries installed already.'''))
try:
  import MySQLdb
except ImportError, e:
  raise ImportError('%s\n%s' % (e, '''You should get MySQLdb from
    http://sourceforge.net/projects/mysql-python. You will need MySQL with
    server binaries installed already.'''))
try:
...
def connect_postgresql():
  try:
    return pgdb.connect(**DB_CREDENTIALS['goship(cchdotest)'])
  except pgdb.Error, e:
    raise IOError("Database error: %s" % e)
def connect_pg_cchdo_data():
  try:
    return pgdb.connect(**DB_CREDENTIALS['goship(cchdo_data)'])
  except pgdb.Error, e:
    raise IOError("Database error: %s" % e)
def connect_mysql():
  try:
    return MySQLdb.connect(**DB_CREDENTIALS['watershed(cchdo)'])
...
def read_arbitrary(filename):
  ...
  if filename.endswith('su.txt'):
    ...
    bottle.woce.woce(datafile).read(handle)
  elif filename.endswith('hy1.csv'):
    bottle.exchange.exchange(datafile).read(handle)
  elif filename.endswith('nc_hyd.zip'):
    bottle.zip.netcdf.netcdf(datafile).read(handle)
  elif filename.endswith('ct.zip'):
    ctd.zip.woce.woce(datafile).read(handle)
  elif filename.endswith('ct1.zip'):
    ctd.zip.exchange.exchange(datafile).read(handle)
  elif filename.endswith('nc_ctd.zip'):
    ctd.zip.netcdf.netcdf(datafile).read(handle)
  else:
...
class Parameter:
  def __init__(self, parameter_name, contrived=False):
    if contrived or parameter_name.startswith('_'):
      ...
      try:
        ...
        print >> stderr, 'Falling back to mysql database for parameter info.'
        try:
          self.init_from_mysql(parameter_name)
        except MySQLdb.Error:
          raise EnvironmentError(
              "No databases could be used for parameter verification")
  def init_from_postgresql(self, parameter_name):
    ...
    cursor.execute(('SELECT %s FROM parameters '
  ...
  def init_from_mysql(self, parameter_name):
    ...
    def wrap_column(s):
      return '`%s`' % s
    select = ','.join(map(wrap_column, ('FullName', 'RubyPrecision',
                                        'Description', 'Units', 'Range',
                                        'Unit_Mnemonic', 'Alias',)))
    cursor.execute(('SELECT %s FROM parameter_descriptions '
  ...
  def read_Summary_HOT(self, handle):
    ...
    for line in handle:
      if header:
        ...
        # TODO Reimplement by finding ASCII column edges in header and reading
        # that way. 
        ...
        self.columns['_CAST_TYPE'].append(tokens[4])
        date = datetime.strptime(tokens[5], '%m%d%y')
        self.columns['DATE'].append('%4d%02d%02d' % (date.year, date.month,
                                                     date.day))
...
class DataFile:
  ...
  def create_columns(self, parameters, units):
    for parameter, unit in zip(parameters, units):
      ...
      try:
        self.columns[parameter] = Column(parameter, self.allow_contrived)
      except Exception, e:
        raise e
  ...
  def read_WOCE_data(self, handle, parameters_line,
    ...
    for line in handle:
      ...
      for i, parameter in enumerate(parameters):
        ...
        if datum is -9.0: datum = float('nan')
        woce_flag = None
